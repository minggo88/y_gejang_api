...
# data 폴더 - 하위폴더 제외합니다.
ls ../data/*.php > data_file_list.txt
xgettext --from-code=UTF-8 --default-domain=API --output-dir=. --output=API.pot --join-existing -f data_file_list.txt
xgettext --from-code=UTF-8 --default-domain=API --output-dir=. --output=API.pot --join-existing -f data_file_list.txt --keyword=__ --keyword=_e
rm data_file_list.txt
# lib 폴더 - 하위폴더 제외합니다.
